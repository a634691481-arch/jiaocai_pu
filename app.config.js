...
export default {
  ...
  checkTokenPages: {
     ...
     */
    mode: 2,
    list: ['/pages/index/*', '/pages/login/index', '/pages/category/*', '/pages/textbook/*', '/pages/my/privacy'],
  },
  checkSharePages: {
    /**
     * 如果 mode = 0 则不做处理
  ...
  interceptor: {
    ...
    fail: function (obj) {
      ...
      if (res.code === -1) {
        vk.alert('登录已过期，请重新登录', () => vk.reLaunch('/pages/login/index'))
        return false
      }
      const msgMap = {
        401: '登录已过期，请重新登录',
      ...
      vk.toast(msgMap[res.code] || res.msg || '请求失败，请稍后重试')
      return false
    },
  },
}