  ...
  <yy-paging v-model="state.dataList" @query="queryList" ref="paging" @scroll="scroll" v-bind="pagingConfig">
    <view class="flex flex-col gap-4 p-4">
      ...
      <view
        ...
        style="background: linear-gradient(135deg, #ffffff, #fafafa); box-shadow: 0 4rpx 24rpx rgba(0, 0, 0, 0.04)"
      >
        <view
          class="-top-8 -right-8 opacity-6 absolute w-24 h-24 rounded-full"
        ...
        <view v-if="userInfo._id" class="relative z-10 flex items-center gap-4">
          ...
          <view class="flex-1">
            ...
            <text class="mt-1 text-xs" style="color: #9ca3af">欢迎使用教材宝 📖</text>
          </view>
        ...
        <view v-else class="relative z-10 flex items-center gap-4" @click="goLogin">
          <view class="w-14 h-14 flex items-center justify-center rounded-full" style="background: #f3f4f6">
            <yy-icon name="ri:user-line" size="24" style="color: #d1d5db" />
          </view>
          <view class="flex-1">
            <text class="text-base font-bold" style="color: #1f2937">点击登录</text>
            <text class="mt-1 text-xs" style="color: #9ca3af">登录后享受完整服务</text>
          </view>
          <view
            class="px-4 py-2 rounded-full relative overflow-hidden active:scale-[0.96] transition-transform duration-200"
          >
            <view class="opacity-10 absolute inset-0" :style="{ backgroundColor: th.primary }" />
            <text class="relative z-10 text-xs font-medium" :style="{ color: th.primary }">去登录</text>
          </view>
        </view>
      </view>
      ...
      <view class="rounded-2xl overflow-hidden" style="background: #ffffff; box-shadow: 0 2rpx 12rpx rgba(0, 0, 0, 0.04)">
        <view
          class="active:bg-gray-50 flex items-center gap-4 p-4 transition-colors duration-150"
        ...
        <view class="active:bg-gray-50 flex items-center gap-4 p-4 transition-colors duration-150" @click="goPrivacy">
          <view
            class="w-9 h-9 rounded-xl flex items-center justify-center"
            :style="{ background: `linear-gradient(135deg, ${th.primaryLight}, ${th.primaryLight})` }"
          >
      ...
      <view v-if="downloadedList.length" class="flex-col gap-3">
        ...
        <view
          ...
          :key="dl._id"
          class="rounded-xl active:bg-gray-50 flex items-center gap-3 p-3 transition-colors duration-150"
          style="background: #ffffff; box-shadow: 0 1rpx 6rpx rgba(0, 0, 0, 0.03)"
          @click="openDownload(dl)"
        >
          <view class="shrink-0 flex items-center justify-center w-10 h-10 rounded-lg" style="background: #fef3c7">
